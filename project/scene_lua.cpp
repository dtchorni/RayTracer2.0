...
extern "C"
int gr_boolean_cmd(lua_State* L)
{
	GRLUA_DEBUG_CALL;
	...
	data->node = new BoolNode(name,type);
...
int gr_nh_cylinder_cmd(lua_State* L) {
	...
	data->node = new GeometryNode(name, new NonhierCylinder(min,max,radius,pos));
...
int gr_nh_cone_cmd(lua_State* L) {
	...
	data->node = new GeometryNode(name, new NonhierCone(min,max,pos));
...
int gr_lego_cmd(lua_State* L)
{
	...
	bool flat = lua_toboolean(L,4);
	data->node = new GeometryNode(name,new Lego(w,l,flat));
...
int gr_mesh_cmd(lua_State* L)
{
	...
	if( i == mesh_map.end() ) {
		mesh = new Mesh( obj_fname, interpolate );
	} else {
		mesh = i->second;
	}
...
extern "C"
int gr_proceduralLego_cmd(lua_State* L){
	GRLUA_DEBUG_CALL;
	...
	int brick_count = int(lua_rawlen(L, 5));
	luaL_argcheck(L, brick_count >= 1, 5, "Bricks expected: (w,l)");

	std::vector<Lego *> bricks;
...
//		luaL_argcheck(L, ldata != 0, 10, "Light expected");
		...
		double dim[2];
		get_tuple(L, -1, &dim[0], 2);

		bricks.push_back(new Lego(dim[0],dim[1],false));
		lua_pop(L, 1);
	}
	...
	int col_count = int(lua_rawlen(L, 6));
	luaL_argcheck(L, brick_count >= 1, 6, "Bricks expected: (w,l)");
	...
	for(int i=1; i<= col_count; ++i) {
		...
		cols.push_back(material);

		lua_pop(L,1);
	}

	bool fill = true;
	fill = lua_toboolean(L,7);

	data->node = new ProcLegoNode(name,w,l,h,bricks,cols,fill);
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"lego",gr_lego_cmd},
  {"proceduralLego",gr_proceduralLego_cmd},
  {0, 0}
};
